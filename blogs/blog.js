...
searchbar.oninput = ()=>{
    ...
    let result_count = 0
    for(let i =0;i<reverse_arr.length;i++){
        isNamePresent = reverse_arr[i].children[0].innerHTML.toLowerCase().includes(searchbar.value.toLowerCase())
        isAuthPresent = reverse_arr[i].children[2].innerHTML.toLowerCase().includes(searchbar.value.toLowerCase())
        if(isNamePresent || isAuthPresent){
            reverse_arr[i].style.display = 'flex'
            no_res.style.display = 'none'
            result_count ++
        }
        else if(!isNamePresent || !isAuthPresent){
